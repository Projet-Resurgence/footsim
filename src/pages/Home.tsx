...
function PitchBackground() {
  return (
    <svg viewBox="0 0 800 520" className="absolute inset-0 w-full h-full pointer-events-none"
      ...
      <polygon points="0,0 260,0 120,520 0,520" fill="white" opacity="0.025" />
      <polygon points="800,0 540,0 680,520 800,520" fill="white" opacity="0.025" />
    </svg>
  );
}

// ── Floating player dots (CSS anim — pas de boucle framer/rAF) ────────────────
const DOTS = [
  { id:1, x:12, y:50, side:'home' }, { id:2, x:22, y:28, side:'home' },
  { id:3, x:22, y:72, side:'home' }, { id:4, x:32, y:18, side:'home' },
  { id:5, x:32, y:42, side:'home' }, { id:6, x:32, y:60, side:'home' },
  { id:7, x:32, y:82, side:'home' }, { id:8, x:44, y:30, side:'home' },
  { id:9, x:44, y:52, side:'home' }, { id:10, x:44, y:72, side:'home' },
  { id:11, x:56, y:50, side:'home' }, { id:12, x:88, y:50, side:'away' },
  { id:13, x:78, y:28, side:'away' }, { id:14, x:78, y:72, side:'away' },
  { id:15, x:68, y:18, side:'away' }, { id:16, x:68, y:42, side:'away' },
  { id:17, x:68, y:60, side:'away' }, { id:18, x:68, y:82, side:'away' },
  { id:19, x:56, y:30, side:'away' }, { id:20, x:56, y:70, side:'away' },
  { id:21, x:44, y:50, side:'away' }, { id:22, x:34, y:50, side:'away' },
] as const;
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {DOTS.map((d, i) => (
        <span
          key={d.id}
          className={`fs-float absolute rounded-full ${d.side === 'home' ? 'bg-accent' : 'bg-gold'} opacity-70`}
          style={{
            left: `${d.x}%`, top: `${d.y}%`, width: 9, height: 9,
            marginLeft: -4.5, marginTop: -4.5,
            animationDuration: `${4.2 + (i % 7) * 0.5}s`,
...
function LiveTicker() {
  ...
  return (
    <div className="inline-flex max-w-full items-center gap-3 sm:gap-4 rounded-full border border-border bg-surface/80 backdrop-blur-sm px-4 sm:px-5 py-2 text-sm shadow-sm">
      <span className="w-2 h-2 rounded-full bg-accent animate-pulse shrink-0" />
      <AnimatePresence mode="wait">
        <motion.span key={idx} className="tabular-nums truncate"
      ...
      <span className="hidden sm:inline text-[10px] uppercase tracking-widest text-muted shrink-0">en direct</span>
    </div>
  );
}
...
function FeatureCard({ icon, title, desc, delay = 0 }: { icon: React.ReactNode; title: string; desc: string; delay?: number }) {
  return (
    <motion.div
      className="group rounded-2xl border border-border bg-surface p-5 sm:p-6 space-y-4 hover:border-accent/50 hover:shadow-subtle-md transition-all duration-300 cursor-default"
...
// ── Stat badge (style tableau d'affichage) ────────────────────────────────────
function StatBadge({ value, label }: { value: string; label: string }) {
  return (
    <motion.div className="text-center"
      initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }} transition={{ duration: 0.4 }}>
      <div className="font-display text-4xl sm:text-5xl font-bold text-accent tabular-nums">{value}</div>
      <div className="text-[11px] sm:text-xs text-muted uppercase tracking-widest mt-1.5">{label}</div>
    </motion.div>
  );
}
...
// ── Styles tactiques présentés (les 12 du moteur) ─────────────────────────────
const STYLE_CARDS = [
  ...
  { name: 'Jeu sur les ailes', desc: 'Débordements et centres. Attaque élargie, milieu sacrifié.' },
  { name: 'Bloc médian', desc: 'Équilibre prudent — compact au milieu, solide derrière.' },
  { name: 'Football total', desc: 'Polyvalence permanente. Léger bonus partout, aucun point faible.' },
];

const FORMATIONS = ['4-3-3','4-4-2','3-5-2','4-2-3-1','5-3-2','4-1-4-1','3-4-3','4-3-2-1','4-5-1','4-4-1-1','3-4-1-2','5-4-1','3-6-1','4-1-2-1-2','3-4-2-1','4-2-2-2','4-2-4'];
...
export default function Home() {
  ...
  const ctaButtons = isLoggedIn ? (
    <>
      ...
      {isAdmin && (
        <Link to="/match">
          <Button variant="outline" className="px-6 py-3 text-base">▶ Lancer un match</Button>
        </Link>
      )}
      {!isAdmin && (
    ...
    <>
      ...
      <a href="#decouvrir">
        <Button variant="outline" className="px-6 py-3 text-base">Découvrir ↓</Button>
      </a>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-bg text-text overflow-x-hidden">
      ...
      {/* ── NAV FLOTTANTE ── */}
      <div className="absolute top-0 inset-x-0 z-20 flex items-center justify-between px-4 sm:px-8 py-4">
        <Link to="/" className="flex items-center gap-2">
          <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-accent/10 text-accent">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="h-4.5 w-4.5" width="18" height="18">
              <circle cx="12" cy="12" r="9" />
              <path d="M12 7.5 8 10.4l1.5 4.6h5L16 10.4 12 7.5ZM12 3v4.5M4 9.5l4 .9M20 9.5l-4 .9M6.5 19.5l3-4.5M17.5 19.5l-3-4.5" />
            </svg>
          </span>
          <span className="font-display text-xl">Foot<span className="text-accent">Sim</span></span>
        </Link>
        {isLoggedIn ? (
          <Button size="sm" variant="soft" onClick={() => navigate(isAdmin ? '/dashboard' : '/my-team')}>
            {isAdmin ? 'Dashboard' : 'Mon équipe'}
          </Button>
        ) : (
          <a href={buildDiscordAuthUrl()}>
            <Button size="sm" variant="soft" className="flex items-center gap-1.5">
              <IconDiscord /> <span className="hidden sm:inline">Connexion</span>
            </Button>
          </a>
        )}
      </div>
      ...
      <section ref={heroRef} className="relative min-h-[100svh] flex flex-col items-center justify-center overflow-hidden">
        ...
        <motion.div className="relative z-10 flex flex-col items-center gap-7 sm:gap-8 px-4 sm:px-6 text-center" style={{ opacity: heroOpacity }}>
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }} className="max-w-full">
            <LiveTicker />
          </motion.div>

          <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.7 }} className="space-y-4 sm:space-y-5">
            <h1 className="font-display text-6xl sm:text-7xl md:text-8xl lg:text-9xl tracking-tight leading-none">
              Foot<span className="text-accent">Sim</span>
            </h1>
            <p className="text-muted text-base sm:text-lg md:text-xl max-w-lg mx-auto leading-relaxed">
        ...
        <motion.div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10"
          ...
          <div className="w-5 h-8 rounded-full border-2 border-muted/25 flex items-start justify-center pt-1.5">
            <div className="w-1 h-2 rounded-full bg-muted/40 fs-float" style={{ animationDuration: '1.8s' }} />
          </div>
        </motion.div>
      </section>

      {/* ── STATS ── */}
      <section className="py-12 sm:py-16 px-4 sm:px-6 border-y border-border/30 bg-surface/30">
        <div className="max-w-3xl mx-auto grid grid-cols-3 gap-4 sm:gap-8 md:gap-16">
          <StatBadge value="97" label="Cultures" />
          <StatBadge value="12" label="Styles tactiques" />
          <StatBadge value="17" label="Formations" />
        </div>
      </section>

      {/* ── FEATURES ── */}
      <section id="decouvrir" className="py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto space-y-10 sm:space-y-14">
          <motion.div className="text-center space-y-3"
            ...
            <h2 className="font-display text-3xl sm:text-4xl md:text-5xl">Tout pour simuler le football</h2>
          ...
          <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <FeatureCard icon={<IconTactic />} title="12 styles tactiques" delay={0.24}
              desc="Possession, gegenpressing, tiki-taka, jeu sur les ailes, football total… Chaque style modifie réellement le déroulement du match — et les affrontements de styles comptent." />
      ...
      {/* ── ENGINE DETAIL ── */}
      <section className="py-16 sm:py-24 px-4 sm:px-6 bg-surface/30 border-y border-border/30">
        <div className="max-w-5xl mx-auto space-y-10 sm:space-y-14">
          <motion.div className="text-center space-y-3"
            initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <SectionLabel>Sous le capot</SectionLabel>
            <h2 className="font-display text-3xl sm:text-4xl">Un moteur de jeu sérieux</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6">
            {[
              ...
              {
                title: 'Météo, arbitres & fatigue',
                desc: 'Chaque match a son arbitre (50 profils) et sa météo selon la zone climatique. La fatigue s\'installe après l\'heure de jeu, les équipes menées poussent en fin de match — le contexte change tout.',
                delay: 0.2,
              },
              {
                title: 'Consignes & plan B',
                desc: 'Tireurs désignés, capitaine, bascules tactiques automatiques quand le score tourne mal, contre-tactiques préparées face à un adversaire précis. Votre banc réagit sans vous.',
                delay: 0.3,
              },
            ].map(({ title, desc, delay }) => (
              <motion.div key={title}
                className="rounded-2xl border border-border bg-surface p-5 sm:p-6 space-y-3"
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto space-y-10 sm:space-y-14">
          <motion.div className="text-center space-y-3"
            ...
            <h2 className="font-display text-3xl sm:text-4xl">12 philosophies tactiques</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 sm:gap-3">
            {STYLE_CARDS.map(({ name, desc }, i) => (
              <motion.div key={name}
                className="rounded-xl border border-border bg-surface px-3.5 sm:px-4 py-3 space-y-1 transition-colors hover:border-accent/40"
            ...
            <motion.div
              className="col-span-2 md:col-span-3 rounded-xl border border-accent/30 bg-accent/5 px-4 sm:px-5 py-4 flex items-start gap-3 sm:gap-4 hover:border-accent/60 transition-colors"
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 bg-surface/30 border-y border-border/30">
        <div className="max-w-5xl mx-auto space-y-10 sm:space-y-14">
          <motion.div className="text-center space-y-3"
            ...
            <h2 className="font-display text-3xl sm:text-4xl">17 schémas tactiques + éditeur libre</h2>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-2">
            {FORMATIONS.map((f, i) => (
              <motion.div key={f}
                className="rounded-lg border border-border bg-surface px-2 sm:px-3 py-3 text-center font-display font-bold text-sm hover:border-accent/50 hover:text-accent transition-colors cursor-default"
                initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <motion.div
            className="rounded-2xl border border-accent/30 bg-accent/5 p-5 sm:p-6 flex flex-col md:flex-row items-start md:items-center gap-4 sm:gap-5"
            initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6">
        <div className="max-w-4xl mx-auto space-y-10 sm:space-y-14">
          <motion.div className="text-center space-y-3"
            ...
            <h2 className="font-display text-3xl sm:text-4xl">Simple à prendre en main</h2>
          </motion.div>
      ...
      <section className="py-16 sm:py-28 px-4 sm:px-6">
        <motion.div
          className="relative max-w-3xl mx-auto text-center space-y-8 overflow-hidden rounded-3xl border border-accent/25 px-5 sm:px-10 py-12 sm:py-16"
          style={{ background: 'linear-gradient(160deg, rgb(var(--accent-rgb) / 0.10), rgb(var(--accent-rgb) / 0.03) 55%, rgb(var(--gold-rgb) / 0.07))' }}
          initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <div className="space-y-4">
            <h2 className="font-display text-4xl sm:text-5xl md:text-6xl">Prêt à jouer ?</h2>
            <p className="text-muted text-base sm:text-lg">Guidez votre nation vers la gloire sur FootSim.</p>
          </div>
      ...
      {/* ── FOOTER ── */}
      <footer className="border-t border-border/30 py-6 px-4 sm:px-6 flex flex-col sm:flex-row items-center gap-2 sm:gap-0 sm:justify-between text-xs text-muted/40">
        <span>FootSim · Projet Résurgence · {new Date().getFullYear()}</span>
        <span>97 cultures · 12 styles · 17 formations</span>
      </footer>
    </div>
  );
}
